...
- The phone on the first Benefits card: its box is centred on the card (0 px off) with its bottom on the card's bottom edge; on desktop the box is 56 % of the card's height and 72 % of its width and the phone fits it bottom-centred, so it draws 239 × 362 px at 1440 × 900 and 188 × 284 px at 1024 × 760 (limited by height), and 198 × 300 px on a 354 × 500 px stacked card; the body copy ends 137 px above it at 1440 × 900 and 52 px above it at 1024 × 760 in English; the mirror card and its phone box equal their grid twins; the `next/image` request is answered 200 as AVIF; alt text present in both locales; the stacked card is 500 px; the pin's hand-over, landing and zero layout shift are unchanged.
...
**Phone mockup** (`public/mockups/points-phone.webp`) stands at the foot of the first Benefits card: the Figma community mockup "Matte iPhone Mockups - 2021 Updated" (node 55977:6896, the matte iPhone 13 template with the WalaOne map screen placed in it). Figma's PNG export bakes the page background and the drop shadow in, so `npm run render:phone` (`scripts/render/build-phone-mockup.mjs`) cuts the phone out with the template's own silhouette path (kept with the screen mask in `scripts/render/src/phone-mockup.json`, taken from the SVG export), recolours the body — the path the template names "[Change this color]" — from light blue to `--color-phone-frame` in OKLab so the matte shading and the neutral speaker and camera details survive, keeps the top three quarters, which is all the card shows, and writes the WebP (848 × 1283, 65 kB). The script's input is the node's 2× PNG export at `.render/figma/phone-mockup@2x.png` (git-ignored; re-export the node when it changes). `ART.benefits.cards` marks the entry `kind: 'phone'` and `BenefitCard` fits it bottom-centred into a box 56 % of the card's height and up to 72 % of its width on desktop (60 % and 64 %, capped at 240 px, when stacked, with a 500 px minimum card height), so the phone scales with the card and stays clear of the copy. Attribute the community file per its licence before launch.

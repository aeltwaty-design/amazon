...
**Hero Lottie** (`public/lottie/hero-illustration.json`, 105 kB) is the design-supplied "Share_Megaphone" sticker from the Neo Brutalism Lottie pack (cyan, pink and yellow-green with a white halo). Its 512 × 512 canvas is mostly margin, so `HERO_LOTTIE.crop` in `lib/art.ts` carries the drawn region (measured over every frame, halo included) and `HeroLottie` shows it through a custom `viewBox`; the headline box takes that region's aspect. The co-branded phone animation assembled by `npm run render:lottie` from the three hero sprites (base64 WebP layers, 5 s loop) is kept at `public/lottie/hero-cobrand.json` as an alternative: point `HERO_LOTTIE.src` at it and drop the crop. Either way it goes through `HeroLottie`, so reduced motion shows the first frame.
